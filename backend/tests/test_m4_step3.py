...
def _make_in_memory_session() -> tuple[Session, object]:
    ...
    # Reload the repository AFTER models so it picks up fresh class references.
    # M7 Step 5: _evaluate_maintenance uses MaintenanceScheduleRepository; the
    # chained joinedload(StockInstance.definition) must use the same StockInstance
    # class that MaintenanceSchedule.instance links to (reloaded just above).
