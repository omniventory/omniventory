...
- **Backend** — Python **3.13**, package manager **uv**; **FastAPI**, **SQLAlchemy 2.0** (typed), **Alembic** (migrations), **SQLite** (default); `pydantic-settings` for config, **argon2** for passwords; **apscheduler** (scheduler), **paho-mqtt** (MQTT bridge), **httpx** (outbound webhook). Quality tools: **ruff** (lint + format), **mypy** (type-check), **pytest**. App factory: `app.main:create_app` (no import side-effects). Auth = opaque server-side **session cookies**; business logic stays in the app/service layer (no SQL-side logic), all DB access via the repository + `RequestContext` layer.
